...
" VS:
"  1. Stolem from ruby
"  2. Deleted not applicable
"  3. Changed multline argument position
"  4. Added default indent = keep indent of current line
...
"  Decho "Default case matched"
  return indent(indent_info.clnum)
endfunction
  ...
  if col > 0
    ...
    if searchpair(escape(bracket_pair[0], '\['), '', bracket_pair[1], 'bW', s:skip_expr) > 0
      if closing_bracket == ')' && col('.') != col('$') - 1
        ...
        let ind = indent(s:GetMSL(line('.'))) + 1
      endif
    endif
